...
- **Shutdown**: `Close(ctx)` returns every session still tracked. Call it right before the process exits, or a session whose return was still in flight stays counted against the organization until it expires on the session server
